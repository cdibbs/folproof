...
box: BOX with EOL clause_list EOL? DEBOX
...
e_iff	: e_iff IFF e_imp
	{ $$ = ['iff', $e_iff, $e_imp]; }
	| e_imp
	{ $$ = $1; }
	;

e_imp	: e_exists IMPLIES e_imp
	{ $$ = ['->', $e_exists, $e_imp]; }
	| e_exists
	{ $$ = $1; }
	;

e_exists	: EXISTS ID e_exists
	{ $$ = ['exists', $ID, $e_exists]; }
	| e_forall
	{ $$ = $1; }
	;

e_forall	: FORALL ID e_forall
	{ $$ = ['forall', $ID, $e_forall]; }
	| e_or
	{ $$ = $e_or; }
	;
...
e_and	: e_and AND e_eq
	{ $$ = ['and', $e_and, $e_eq]; }
	| e_eq
	{ $$ = $1; }
	;

e_eq	: e_eq EQUALS e_not
	{ $$ = ['=', $e_eq, $e_not]; }
	| e_not
	{ $$ = $1; }
	;
...
term	: ID LPAREN term_list RPAREN
	{ $$ = ['id', $ID, $term_list]; }
	| ID LPAREN RPAREN
	{ $$ = ['id', $ID, []]; }
	| ID
